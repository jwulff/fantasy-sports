...
| [commissioner-cookie-writes-any-teams-lineup.md](commissioner-cookie-writes-any-teams-lineup.md) | The commissioner's cookie moved another team's lineup with `isLeagueManager: false` — ownership is a client-side guard regardless of role, LM actions are a separate gated capability, and no cassette may pretend ESPN rejects it |
