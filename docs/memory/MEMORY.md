...
| [espn-lineup-write-authority-is-broader-than-team-ownership.md](espn-lineup-write-authority-is-broader-than-team-ownership.md) | The league creator's cookie moved a non-owned team's lineup with `isLeagueManager: false` — ownership is a client-side guard regardless of role, LM actions are a separate gated capability, no cassette may pretend ESPN rejects it, and the non-privileged case is unverified |
| [commissioner-authority-is-two-flags-in-mnav.md](commissioner-authority-is-two-flags-in-mnav.md) | `mTeam` carries no authority flag; `mNav` carries `isLeagueCreator` *and* `isLeagueManager`, and the founder has only the first — read both, or `fan.api`'s `groupManager` |
